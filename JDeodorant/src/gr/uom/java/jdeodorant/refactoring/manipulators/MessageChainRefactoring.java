...
import gr.uom.java.jdeodorant.refactoring.views.MCPreviewWizard;
...
public class MessageChainRefactoring {
	...
	public static void modifyMethodInvocationFile(ICompilationUnit compUnitOfMethodInvocation, String strOfMethod) {
		try {
			...
			modifyCompUnit(workingCopyOfMethodInvocation, bufferOfMethodInvocation, modifyPosition, 0, strOfMethod, false);
	 ...
	 **/
	public static boolean modifyCompUnit(ICompilationUnit workingCopy, IBuffer buffer, int startPos, int len, String stringForChange, boolean preview) {
		...
		if(preview) {
		// Preview
		MCPreviewWizard wizard = new MCPreviewWizard(workingCopy, origin, refactor);
		WizardDialog dialog = new WizardDialog(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(),wizard);
		dialog.open();
		return wizard.getFlagCancel();
		} else {
			 try {
